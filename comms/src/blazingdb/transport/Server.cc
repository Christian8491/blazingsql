...
collect_gpu_message(
	void * socket, int gpuId, void (*read_tpc_message)(std::vector<unsigned long long int>, void *, int, buffer_container_type &)) {
	zmq::socket_t * socket_ptr = (zmq::socket_t *) socket;
	// begin of message
	Message::MetaData message_metadata = read_metadata<Message::MetaData>(socket);
	...
	std::vector<unsigned long long int> buffer_sizes(buffer_sizes_size);
	blazingdb::transport::io::readFromSocket(socket, (char *) buffer_sizes.data(), buffer_sizes_size * sizeof(unsigned long long int));
